...
function Statements({
  ...
  const pool = (items) => {
    ...
    items.forEach((item) => {
      if (
        !pooled.length ||
        pooled[pooled.length - 1][0].taxon !== item.taxon ||
        pooled[pooled.length - 1][0].character !== item.character
      ) {
        pooled = [...pooled, [item]];
      } else {
  ...
  const addObjects = (items) => {
    return items.map((s) => {
      s["taxonObject"] = flattenTaxa(deepClone(taxa)).find(
        (t) => t.id === s["taxon"]
      );
      if (!!s["taxonObject"]) {
      ...
      if (!!s["characterObject"]) {
        ...
        if (!!s["value"]) {
          s["stateObject"] = deepClone(
            character["states"].find((t) => t.id === s["value"])
          );
        }
      }
      return s;
  ...
  return (
    <div>
      ...
      <Alert severity="info">
        ...
        Statements are knowledge about taxa needed to separate them from other
        taxa. In Clavis, this knowledge is a connection between a taxon (such as
        a species), a character (e.g. "Color"), a state (e.g. "Red"), and a
        frequency ("always", "in some cases", or "never"). This way, one can
        make statements reflecting that "The color of a polar bear is always
        white" and "The color of a brown bear is always brown" so the two can be
        distinguished. Through three additional statements it can be stated that
        "The color of an arctic fox is, in some cases, white", "The color of an
        arctic fox is, in some cases, brown", and "The color of an arctic fox
        is, in some cases, grey". The combination of these example statements
        makes it so that more statements with other characters are needed to
        distinguish white arctic foxes from polar bears; for example using a
        character regarding their size. Redundant statements are allowed and in
        fact encouraged, so the number of toes can be added as an alternative to
        distinguish polar bears from arctic foxes.
      </Alert>
      ...
      {!!languages.length && !!statements.length && (
        <TextField
          ...
          onChange={(e) => {
            setFiltered(
              removeObjects(
                search(addObjects(deepClone(statements)), e.target.value)
              )
            );
          }}
        />
      ...
      {!taxa.length && (
        <Alert severity="error">Create at least one taxon first.</Alert>
      )}
      {!characters.length && (
        <Alert severity="error">Create at least one character first.</Alert>
      )}
